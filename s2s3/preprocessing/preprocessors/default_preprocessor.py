...
class DefaultPreprocessor(object):
    ...
    def run_case_npy(self, data: np.ndarray, seg: Union[np.ndarray, None], properties: dict,
                     ...
                     dataset_json: Union[dict, str],  mask_ratio=0.35):
        ...
        #data, mask, ids_restore = self.random_masking(data, mask_ratio)
        if isinstance(data, torch.Tensor):
            data = data.cpu().numpy()  # Convert Tensor back to numpy for normalization
    ...
    def random_masking(self, x, mask_ratio):
        """
        Perform random masking while maintaining spatial structure.
        x: [N, C, H, W], the input data, expected to be a numpy array.
        """
        # Convert numpy array to torch Tensor if necessary
        
        N, C, H, W = x.shape
        L = H * W  # Total number of pixels in each image (H * W)
        len_keep = int(L * (1 - mask_ratio))  # Number of pixels to keep

        # Generate random noise for shuffling per image and shuffle pixels spatially
        noise = torch.rand(N, H, W, device=x.device)  # Random noise with the shape of H, W
        ids_shuffle = torch.argsort(noise.view(N, -1), dim=1)  # Flatten noise and sort indices
        ids_restore = torch.argsort(ids_shuffle, dim=1)

        # Mask out pixels spatially while keeping H, W dimensions intact
        mask = torch.ones([N, H, W], device=x.device)
        flat_ids_keep = ids_shuffle[:, :len_keep]  # Select a subset of pixels to keep
        mask = mask.view(N, -1)
        mask[:, flat_ids_keep] = 0  # Set the selected subset to 0 (i.e., keep those pixels)

        mask = mask.view(N, H, W)  # Reshape mask back to [N, H, W]
        if isinstance(x, np.ndarray):
            x = torch.from_numpy(x).to(x.device)  # Convert to Tensor and move to the correct device
        # Apply the mask to the input data
        x_masked = x * mask.unsqueeze(1)  # Keep selected pixels and set the rest to 0
...
def example_test_case_preprocessing():
    ...
    plans_file = '/home/joyce/nnUNet/nnUNet_preprocessed/Dataset110_Tiger/nnUNetPlans.json'
    dataset_json_file = '/home/joyce/nnUNet/nnUNet_preprocessed/Dataset110_Tiger/dataset.json'
    ...
    # 指定图像文件夹路径
    image_folder = '/home/joyce/nnUNet/nnUNet_raw/Dataset110_Tiger/imagesTr/'

    # 获取所有图像的路径
    input_images = [os.path.join(image_folder, img) for img in os.listdir(image_folder) if img.endswith('.png')]
    configuration = '2d'
