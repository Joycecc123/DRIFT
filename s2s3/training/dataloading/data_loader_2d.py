...
class nnUNetDataLoader2D(nnUNetDataLoaderBase):
    def generate_train_batch(self):
        ...
        for j, current_key in enumerate(selected_keys):
            ...
            # print(f"{j}-{current_key}: seg shape: {seg.shape}, self.seg_shape: {self.seg_shape}")
            if seg.shape[0] != 1:
                seg = seg[0, ::]
                seg = np.expand_dims(seg, axis=0)
                #print(f"update shape: {j}-{current_key}: seg shape: {seg.shape}, self.seg_shape: {self.seg_shape}")
