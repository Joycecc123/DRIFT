...
def compute_metrics_on_folder(folder_ref: str, folder_pred: str, output_file: str,
    ...
    计算文件夹中预测结果的评估指标
    Args:
        folder_ref: 参考分割的文件夹路径
        folder_pred: 预测分割的文件夹路径
        output_file: 输出JSON文件的路径
        image_reader_writer: 图像读写器
        file_ending: 文件扩展名
        regions_or_labels: 需要评估的区域或标签列表
        ignore_label: 忽略的标签值
        num_processes: 并行处理的进程数
        chill: 是否允许预测文件夹缺少部分文件
    Returns:
        包含评估指标的字典
    """
    if output_file is not None:
    ...
    if not os.path.exists(folder_ref):
        raise ValueError(f"Reference folder does not exist: {folder_ref}")
    if not os.path.exists(folder_pred):
        raise ValueError(f"Prediction folder does not exist: {folder_pred}")
        
    # 获取文件列表
    print(f"\nScanning folders for {file_ending} files...")
    files_pred = subfiles(folder_pred, suffix=file_ending, join=False)
    files_ref = subfiles(folder_ref, suffix=file_ending, join=False)
    
    # 打印文件夹内容信息
    print(f"\nFolder Contents:")
    print(f"Reference folder ({folder_ref}):")
    print(f"- Found {len(files_ref)} files with extension '{file_ending}'")
    if len(files_ref) > 0:
        print(f"- Example files: {files_ref[:3]}")
    
    print(f"\nPrediction folder ({folder_pred}):")
    print(f"- Found {len(files_pred)} files with extension '{file_ending}'")
    if len(files_pred) > 0:
        print(f"- Example files: {files_pred[:3]}")
    
    # 检查预测文件夹中的文件
    if len(files_pred) == 0:
        all_files = os.listdir(folder_pred)
        if len(all_files) > 0:
            file_extensions = set(os.path.splitext(f)[1] for f in all_files)
            print(f"\nWarning: Found {len(all_files)} files with different extensions:")
            print(f"Available extensions: {file_extensions}")
            print(f"First few files: {all_files[:3]}")
            raise ValueError(f"Prediction folder has files but none with extension '{file_ending}'")
        else:
            raise ValueError(f"Prediction folder is empty: {folder_pred}")
    
    # 文件匹配检查
    if not chill:
        missing_files = [f for f in files_ref if f not in files_pred]
        if missing_files:
            ...
            raise ValueError(f"Missing {len(missing_files)} prediction files")
    
    # 获取共同文件
    common_files = sorted(list(set(files_pred).intersection(set(files_ref))))
    if not common_files:
        raise ValueError("No matching files found between prediction and reference folders!")
    
    # 构建完整路径
    files_ref = [join(folder_ref, i) for i in common_files]
    files_pred = [join(folder_pred, i) for i in common_files]
    
    print(f"\nStarting evaluation:")
    print(f"- Processing {len(common_files)} matching files")
    print(f"- Using {num_processes} processes")
    print(f"- Evaluating regions/labels: {regions_or_labels}")
    ...
    if not results:
        raise ValueError("No results were computed!")

    # 过滤有效结果
    valid_results = [r for r in results if r is not None and 'metrics' in r and r['metrics']]
    if not valid_results:
        raise ValueError("No valid results were found after computation!")

    # 计算每个类别的平均指标
    first_result_metrics = valid_results[0]['metrics']
    if not first_result_metrics:
        raise ValueError("First result has no metrics!")
    
    # 获取指标列表
    first_region = list(first_result_metrics.keys())[0]
    metric_list = list(first_result_metrics[first_region].keys())
    ...
    for r in regions_or_labels:
        means[r] = {}
        for m in metric_list:
            values = [i['metrics'][r][m] for i in valid_results if r in i['metrics']]
            means[r][m] = np.nanmean(values) if values else np.nan
    ...
    for m in metric_list:
        ...
        for k in means.keys():
            if k == 0 or k == '0':
                continue
            if means[k][m] is not None and not np.isnan(means[k][m]):
                values.append(means[k][m])
        foreground_mean[m] = np.mean(values) if values else np.nan

    # 处理JSON导出
    [recursive_fix_for_json_export(i) for i in valid_results]
    recursive_fix_for_json_export(means)
    recursive_fix_for_json_export(foreground_mean)

    # 构建结果字典
    result = {
        'metric_per_case': valid_results,
        'mean': means,
    ...
    # 打印摘要
    print("\nEvaluation Summary:")
    print(f"- Processed {len(valid_results)} cases successfully")
    if 'Dice' in foreground_mean:
        print(f"- Overall Mean Dice Score: {foreground_mean['Dice']:.4f}")
